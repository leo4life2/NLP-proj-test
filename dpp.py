...
def run(local_rank, world_size, node_rank):
    global_rank = node_rank * 2 + local_rank
    print(f"Starting process for Rank {global_rank}.")
...
def main():
    ...
    if args.one_node:
        ...
        mp.spawn(run, args=(world_size, node_rank), nprocs=2, join=True, start_method='spawn')
